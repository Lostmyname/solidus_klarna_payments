...
describe KlarnaGateway::Payment::Processing, :klarna_api do
  ...
  context "#klarna_order" do
    it "calls the api to get the order" do
      ...
      allow(payment).to receive(:klarna_gateway).and_return(api_client)
  ...
  context "#refresh!" do
    it "calls the api to update the source" do
      expect(payment.send(:klarna_gateway)).to receive(:get_and_update_source).with(payment.source.order_id).and_return(true)
      payment.refresh!
    end
  end
  ...
  context "#extend_period!" do
    it "calls the api to extend credit authorization period" do
      expect(payment.send(:klarna_gateway)).to receive(:extend_period).with(payment.source.order_id).and_return(true)
  ...
  context "#release!" do
    it "calls the api to release remaining amount" do
      expect(payment.send(:klarna_gateway)).to receive(:release).with(payment.source.order_id).and_return(true)
      payment.release!
    end
  end
